...
| `Line_Following_Display_Distance.ino` | **Distance Measurement**  | Uses the Rotary Encoder on an interrupt pin (D2) to calculate and display total distance traveled.                                     |
...
| `Bluetooth_Controlled_Car.ino`        | **Wireless Control**      | Reads serial commands ('F', 'B', 'L', 'R', etc.) sent from a smartphone remote app via the HC-05 module.                               |

---

## ✅ Key Analysis and Learning Outcomes

- **Timing Accuracy:** The shift from the problematic `delay()` function to the non-blocking **`millis()`** function was crucial for accurate, independent timing.
- **Distance Correction:** The rotary encoder required a pragmatic correction factor (dividing the output by 4) to ensure the calculated distance closely matched the actual distance. The resulting inaccuracy was noted to be less than 2%.
- **Robustness:** To handle reliability issues observed with the Bluetooth connection, the most advanced feature planned was the **merger of Bluetooth control with Obstacle Avoidance**, creating a safer robot that can avoid head-on collisions even if the wireless link drops.
